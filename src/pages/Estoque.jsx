...
export default function Estoque() {
  ...
  const inventory = useMemo(() => [...products]
    ...
    .filter((product) => `${product.nome} ${product.categoria}`.toLocaleLowerCase('pt-BR').includes(search.toLocaleLowerCase('pt-BR'))), [products, search]);
  ...
  return (
    <div>
      ...
      <div className="table-toolbar"><label className="search-field"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar produto" /></label><span>{inventory.length} item(ns)</span></div>
      <div className="table-wrap"><table><thead><tr><th>Produto</th><th>Categoria</th><th>Disponível</th><th>Mínimo</th><th>Preço de venda</th><th>Situação</th></tr></thead><tbody>
        ...
        {!inventory.length && <tr><td className="empty-state" colSpan="6">Nenhum produto em estoque.</td></tr>}
      </tbody></table></div>
    </div>
  );
}
